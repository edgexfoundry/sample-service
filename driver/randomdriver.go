...
func (d *RandomDriver) DisconnectDevice(deviceName string, protocols map[string]models.ProtocolProperties) error {
	d.lc.Info(fmt.Sprintf("RandomDriver.DisconnectDevice: sample-service driver is disconnecting to %s", deviceName))
	return nil
}
...
func (d *RandomDriver) Stop(force bool) error {
	d.lc.Info("RandomDriver.Stop: sample-service driver is stopping...")
	return nil
}
